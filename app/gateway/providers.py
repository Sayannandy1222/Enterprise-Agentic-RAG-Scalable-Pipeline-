...
class OpenAICompatibleProvider:
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        return LLMResponse(
            ...
            usage.get("prompt_tokens", _estimate_tokens(request.prompt)),
...
class GroqProvider(OpenAICompatibleProvider):
    def __init__(self, api_key, model="llama-3.3-70b-versatile", **kwargs):
        super().__init__(
            "groq", api_key, "https://api.groq.com/openai/v1", model, **kwargs
        )


class PortkeyProvider(OpenAICompatibleProvider):
    def __init__(self, api_key, virtual_key, model="llama-3.3-70b-versatile", **kwargs):
        super().__init__(
            "portkey",
            api_key,
