...
try:
    persisted_documents = store.load_documents()

    if persisted_documents:
        bm25.add(persisted_documents)

except Exception as exc:
    print(f"WARNING: failed to hydrate BM25 from Qdrant: {type(exc).__name__}: {exc}")

retrieval = RetrievalService(
    store,
    bm25,
    dense_limit=settings.dense_candidate_limit,
    bm25_limit=settings.bm25_candidate_limit,
    rrf_limit=settings.rrf_candidate_limit,
...
if settings.portkey_api_key and settings.portkey_virtual_key:
    provider = PortkeyProvider(
        settings.portkey_api_key, settings.portkey_virtual_key, settings.groq_model
    )
...
app.state.gateway = LLMGateway(
    ...
    fallback_provider=LocalProvider() if provider else None,
)
...
def ready():
    return {
        "status": "ready",
        "embedding_cache": embedder.stats(),
