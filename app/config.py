...
class Settings(BaseSettings):
    ...
    def validate_candidate_order(self):
        if self.dense_candidate_limit < self.top_k:
            raise ValueError("dense_candidate_limit must be >= top_k")

        if self.bm25_candidate_limit < self.top_k:
            raise ValueError("bm25_candidate_limit must be >= top_k")

        if self.rrf_candidate_limit < self.top_k:
            raise ValueError("rrf_candidate_limit must be >= top_k")

        if self.rerank_candidate_limit < self.top_k:
            raise ValueError("rerank_candidate_limit must be >= top_k")

        if self.rerank_candidate_limit > self.rrf_candidate_limit:
            raise ValueError("rerank_candidate_limit cannot exceed rrf_candidate_limit")

        if (
            self.rrf_candidate_limit
            > self.dense_candidate_limit + self.bm25_candidate_limit
        ):
