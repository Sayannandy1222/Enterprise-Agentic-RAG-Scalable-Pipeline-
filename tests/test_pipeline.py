...
def test_cache_is_bounded_and_clearable():
    ...
    embedder.embed("one"); embedder.embed("two"); embedder.embed("three")
...
def test_api_upload_then_query():
    ...
    uploaded = client.post("/api/v1/documents/upload", files={"file": ("note.txt", b"hybrid retrieval", "text/plain")})
...
def test_gateway_and_context_budget():
    response = LLMGateway().generate(LLMRequest(prompt="Context: alpha beta\nQuestion: explain"))
    assert response.text and response.input_tokens > 0 and response.total_tokens >= response.output_tokens
